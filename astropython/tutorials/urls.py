...
from .views import start_step,intermediate_step,finish_step,single,vote

urlpatterns = patterns('',
    url(r'^(?P<section>resources|snippets|tutorials|series)/create/$',start_step,{'model':EducationalResource},name="creation_start"),
    url(r'^(?P<section>resources|snippets|tutorials)/create/intermediate/(?P<slug>[\w-]+)/$',intermediate_step,name="creation_intermediate"),
    url(r'^(?P<section>resources|snippets|tutorials)/create/finish/(?P<slug>[\w-]+)/$',finish_step,name="creation_finish"),
